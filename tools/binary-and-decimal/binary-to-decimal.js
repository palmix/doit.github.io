document.getElementById("oldNumber").addEventListener("keyup", function () {
		...
		noBadCharacters = noBadCharacters.replace(/[^0-1\s]/g,'');  
	...
	if (theNumbers !== ''){
		theNumbers = theNumbers.replace(/(\r\n|\r)/gm,"\n");
				...
				for(var i=0;i<newCodeArray.length;i++){
					...
					if(newCodeArray[i]){
						binary = newCodeArray[i];
						binaryString = binaryString + binary + '\n';
						decimalString = decimalString + parseInt(binary, 2) + '\n';	
					}else{
		...
		document.getElementById("oldNumber").value = binaryString;
		document.getElementById("newNumber").value = decimalString;
	}
	}else{
